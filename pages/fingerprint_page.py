...
class FingerPrintPage:
    ...
    JOB_TITLE_DROP_DOWN = """//input[@placeholder="Job Title"]"""
    ...
    def is_element_present(self, sb, locator):
        try:
            sb.find_element(locator, timeout=5)
            return True  # Element is present
        except NoSuchElementException:
            return False

    def build_communication_fingerprint(self, sb):
        selected_item = {}
        sb.click(self.DISCOVER_MY_FINGERPRINT_BUTTON)
        ...
        while self.is_element_present(sb, self.NEXT_BUTTON_DISABLED, timeout=5):
            temp_q = sb.find_elements(self.MY_FINGERPRINT_QUESTION_INTENT)
            temp_q_text = [template.text for template in temp_q]
            delimiter = " "
            initial_question_text = delimiter.join(temp_q_text)

            if sb.find_element(self.SELECTION_TITLE_FOR_MULTIPLE_SELECTION, timeout=1):
            ...
            elif sb.find_element(self.SELECTION_ITEM_FOR_ONE_SELECTION, timeout=1):
            ...
            elif sb.find_element(self.SELECTION_ITEM_INDUSTRY_FOR_ONE_SELECTION, timeout=1):
            ...
            elif sb.find_element(self.SELECTION_GROUP_FOR_MULTIPLE_SELECTION, timeout=1):
            ...
            elif sb.find_element(self.SELECTION_ITEM_FOR_MULTIPLE_SELECTION, timeout=1):
        ...
        if sb.find_element(self.JOB_TITLE_DROP_DOWN, timeout=5):
            sb.select_option_by_index(self.JOB_TITLE_DROP_DOWN, 0)
